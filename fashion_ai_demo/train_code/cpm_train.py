...
valid_img_path = os.path.join(rootdir ,valid_image_folder_name)
valid_file_name = os.path.join(rootdir , valid_image_folder_name+"Annotations/train_"+category_name+"_coord.csv")
df_valid = pd.read_csv(valid_file_name)
valid_data = train_input.data_stream(df_valid,config.batch_size,is_train=True , pre_path = valid_img_path)
...
with tf.Session() as sess:
    ...
    for i in range(config.max_iteration):
        ...
        if (i+1) % config.validate_iters== 0:
            ...
            acc_list = np.array([])
            loss_list = np.array([])
            for i_df_valid in np.arange(0,df_valid.shape[0],config.batch_size):
